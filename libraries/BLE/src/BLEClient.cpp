...
BLEClient::~BLEClient()
{
	...
	log_e("BLEClient destructor called");
	clearServices();
	//unregist();
} // ~BLEClient
...
{
	log_e(">> unregist() called");

	if (isConnected())
	{
		log_e("disconnect called");
		disconnect();
	}

	if (m_isRegistered)
	{
		//log_e(m_appId.toString());
		try
		{
			m_semaphoreRegEvt.take("unregist");
			log_e("m_isRegistered");
	...
	esp_ble_gattc_cb_param_t *evtParam)
{
	...
	case ESP_GATTC_OPEN_EVT:
	{
		if (evtParam->open.status == ESP_GATT_OK)
		{
			...
			m_isConnected = true; //i had to add this for notifications, might be a problem with sdkconfig.h?
		}
		if (m_pClientCallbacks != nullptr)
		{
